...
> 最后更新：2026-05-28 — 基础 17/162 ✅ · 进阶 10/75 ✅ · OOP 5/10（3.1~3.5 完成，3.6~3.10 待施工）· 流与文件 1/7（入口节点完成）· STL 0/5 · 泛型 0/4 · 现代C++ 0/7 · 工程化 0/5

### 03 OOP 待施工

| # | 节点 | 内容 | 卡片 | 状态 | 说明 |
|---|------|------|:--:|:--:|------|
| 3.1 | 类与对象 | class vs struct、this 指针、const 成员函数、**this 底层** | 10 | ✅ | 已实现 7 卡。待补 3 卡：this 是编译器隐式参数、const 对 this 类型的影响、\*this 与链式调用原理。吸收原 3.13 前半 |
| 3.2 | 访问控制与封装 | public/private 分工、BankAccount 实战、getter/setter 的度 | 7 | ✅ | 完整 |
| 3.3 | static 成员 | 静态数据成员、静态成员函数、对象计数器、**:: 底层** | 10 | ✅ | 已实现 7 卡。待补 3 卡：:: 类成员访问（Student::getCount()）、:: 命名空间（std::cout）、:: 全局作用域。吸收原 3.13 后半 |
| 3.4 | 构造与析构 | 构造函数、初始化列表、explicit、析构、RAII、构造析构顺序、局部 static | 13 | ✅ | 完整 |
| 3.5 | 拷贝控制 | 浅拷贝灾难、深拷贝、Rule of Three、**<< 重载** | 11 | ✅ | 已实现 9 卡。待补 2 卡：operator<< + friend（让 cout 打印自定义对象）、operator==。吸收原 3.6 核心部分，裁掉 operator+ 和优先级讨论 |
| 3.6 | 继承 | is-a、三种继承访问修饰符、构造析构顺序、组合 vs 继承 | 7 | ⬜ | |
| 3.7 | 多态 | virtual、vtable、override/final、对象切片、虚析构、纯虚函数 | 9 | ⬜ | |
| 3.8 | 设计原则 | 组合优于继承、面向接口、OOP 不是银弹 | 5 | ⬜ | |
| 3.9 | 实战：成绩管理系统 | Student+Campus+GradeBook→继承→多态→<<重载→改错 | 7 | ⬜ | |
| 3.10 | 分文件+编译链接 | .h/.cpp 分离、include guard、预处理→编译→链接、undefined reference、static internal linkage | 7 | ⬜ | 原 3.7+3.8 合并，工程技能附录。可跳过 |
...
| 链接属性 | 03 OOP | 3.10 分文件+编译链接 | static 全局变量/函数：internal linkage（文件作用域限制，工程附录） |
...
| 03  | 面向对象           | 10     | ~82 卡 | 🔨 5/10 — 类与对象(+this) · 访问控制与封装 · static成员(+::) · 构造析构 · 拷贝控制(+<<重载) · 继承 · 多态 · 设计原则 · 实战 · 分文件+编译链接(附录) |
...
> OOP 主线一气呵成：**造类**（3.1-3.5 融入 this/::/<<）→ **类间关系**（3.6-3.7 继承/多态）→ **落地**（3.8-3.9 设计原则/实战）。3.10 分文件+编译链接作为工程附录，按需取用。

### 3.3 static 成员 (7 卡)

> 3.2 讲完"每个对象有自己的 private 数据"，3.3 自然追问"那有没有不属于任何对象、整个类共享的数据？"。
> 静态数据成员的声明+类外定义是两个独立语法点，各需概念卡+代码卡；再加注意事项和练习，共 7 张。
...
| c6 | concept | 析构函数——对象的"临终结算" | 语法：`~类名() { }`，无参无返回。调用时机：栈对象出 `}` / delete 堆对象 / 程序结束（全局和 static）。不写 → 编译器生成空析构。必须写 → 对象持有外部资源（堆内存、文件句柄、锁）。虚析构预告："如果类会被继承，析构函数前加 virtual。原因在 3.7 多态展开，现在先记住规则。" 行为总结：**"走之前把烂摊子收拾干净"** |
...
| c9 | concept | 谁先构造、谁后析构 | 三个维度：①成员 vs 自身——声明顺序 a→b→函数体，析构逆序 ②全局 vs 局部——全局对象在 main 之前构造， main 结束后析构；局部对象进作用域构造、出作用域析构 ③基类 vs 派生类（3.6 继承 预告）——基类先构造→派生类后，析构逆序。口诀：**"构造：外→内、全局→局部；析构反过来"** |
...
- **后置：** 3.5 拷贝控制（拷贝构造是特殊的构造函数）、3.6 继承（基类构造析构顺序）、3.7 多态（虚析构展开）



---

### 3.5 拷贝控制 + << 重载 (11 卡) — 9 已实现 + 2 待补

> 3.4 讲了构造和析构。一个自然的问题：对象之间能互相拷贝吗？怎么拷贝？本节固定用 IntArray 类贯穿，c1-c9 已实现。c10-c11 吸收原 3.6 运算符重载核心（<<、==），裁掉 operator+ 和优先级讨论。
...
| c10 | concept | operator<< — 让 cout 打印自定义对象 | ⬜ `operator=` 讲完自然引出"那 cout 怎么打印我的对象？"。`friend ostream& operator<<(ostream&, const T&)` — 必须用友元因为左操作数是 ostream |
| c11 | concept | operator== — 两个对象怎么比 | ⬜ `bool operator==(const T&) const` — 比较不修改。什么时候重载？"需要比较两个对象的内容是否相等时" |

### 关联

- **前置：** 3.4 构造与析构（析构释放资源是理解浅拷贝危害的前提）
- **后置：** 3.6 继承（拷贝构造在继承中的行为）、3.7 多态（虚析构展开）、3.9 实战（<< 重载打印成绩单）

---

### 3.6 继承 (7 卡) ⬜

> 从"一个类怎么写"跨到"类与类之间的关系"。承上——构造/析构/访问控制全部用上；启下——虚函数在 3.7 展开。
...
| c6 | concept | 继承 vs 组合 | 继承="is-a"（狗是动物），组合="has-a"（车有引擎）。3.8 展开设计原则 |
| c7 | practice | 两道题 | ① 私有父类成员能被子类直接访问吗？（不能）② 构造顺序：父类先还是子类先？（父类先） |

### 关联

- **前置：** 3.4 构造与析构、3.5 拷贝控制
- **后置：** 3.7 多态（virtual、虚析构）、3.8 设计原则（组合优于继承）
...
> 收束节点——把 3.1~3.8 的概念串成一个活的程序。不要求手写新代码，每张卡展示成品+高亮关键行+解释串了哪个知识点。
...
| c2 | code | Student + Course——组合关系 | 用 3.1~3.4 的知识写完整 Student 和 Course。Course 存课程名+成绩；Student 用 `vector<Course>` 组合多门课——体现 3.8 has-a。构造用初始化列表 |
...
| c4 | code | 继承：本科生 vs 研究生 | UndergradStudent 和 GradStudent 继承 Student（3.6）。研究生+thesisScore，calcGPA() 算法不同。子类构造调父类构造，override 标记重写（3.7 c5） |
| c5 | code | 多态+虚析构：统一打印成绩报告 | GradeBook 用 `vector<unique_ptr<Student>>`。`printReport()` 遍历指针多态调用（3.7）——一行代码打出不同格式。基类 `virtual ~Student() = default;`（3.7 c7） |
| c6 | code | 重载 << ——让代码说人话 | 为 Student 重载 `operator<<`（3.5 c10），让 `cout << s` 直接输出格式化成绩单。配合 iomanip 整齐排版。完整 ~60 行，读完有"原来 OOP 是这么用的"的全局感 |
...
- **前置：** 全部 OOP 节点（3.1-3.8）
### 3.10 分文件+编译链接 (7 卡) ⬜ — 工程附录
...
- **前置：** 3.3 static 成员（`int Student::count = 0` 类外定义语法）、3.5 拷贝控制（头文件里 `= delete` 禁止拷贝）
