...
> 最后更新：2026-05-24 — 完成基础模块 14 节点 + 进阶模块指针基础
...
| 2 | 变量声明 | `variables.ts` | 12 卡 | 变量概念、基本类型、扩展整数类型、sizeof、初始化与赋值、标识符命名规则、常见关键字清单 |
| 3 | 运算符与表达式 | `operators.ts` | 9 卡 | 算术运算符、赋值/复合赋值、自增自减（前缀 vs 后缀）、取余练习 |
| 4 | 输入与输出 | `io.ts` | 12 卡 | cout 输出、cin 输入、综合示例、转义字符、endl vs \n |
| 5 | 常量 | `constants.ts` | 5 卡 | const 声明、编译期 vs 运行期(constexpr)、#define 宏定义、练习 |
...
14/14 节点，112 张卡片。函数入门为收官节点，把之前所有知识串联起来。
...
| 02 | 进阶 | 5 | ~26 卡 | ✅ 指针 · ⬜ 引用 / 动态内存 / 函数重载 / 文件IO |
...
> 从原 1.8 条件分支中拆分出来作为前置。学生必须先理解 > < == != && || ! 和 = vs == 陷阱，再学 if/else 才不会被两个新概念同时轰炸。

| # | 卡类型 | 标题 | 内容要点 |
|---|--------|------|---------|
| c1 | concept | 比较运算符 | `> < >= <= == !=` 六个运算符，都返回 bool；判断标准："这个比较成立吗？" |
| c2 | code | 比较运算符示例 | 5 和 3 的六种比较，cout 输出 1/0（true/false） |
| c3 | concept | 逻辑运算符 | `&&`（与）、`||`（或）、`!`（非）真值表；一句话：&&→"两边都是真？"，\|\|→"至少一边是真？"，!→"反过来" |
| c4 | code | 逻辑运算符示例 | age>=18 && hasID、hasCard \|\| hasID、!isWeekend |
| c5 | concept | = 和 == 的区别 | = 是赋值"放进去"，== 是比较"相等吗？"；对照例子：`x=10` vs `x==10`；`(x=5)` 返回值 5 而非比较结果 |
| c6 | practice | `10 > 7` 的结果？ | choice: true / false / 10 / 7 → true |
...
| # | 卡类型 | 标题 | 内容要点 |
|---|--------|------|---------|
| c1 | concept | if/else — 让程序做选择 | 语法、判断标准"条件表达式是 true 还是 false？"、对照例子（`score>=60` ✓ vs `score=60` ✗） |
| c2 | code | 成绩等级判断 | 90→A, 80→B, 70→C, 60→D, 其余→F，完整 if/else if 阶梯 |
| c3 | **animation** | **if-else-if-ladder** | BranchPlayer：条件链依次检查 — 第一个不成立跳过 → else if 成立执行，else 跳过。展示"从上到下依次检查，第一个成立就停" |
| c4 | concept | switch — 等值多分支 | 适用场景、break 防穿透、default、不能用于 string/double；判断标准"变量和哪些确定的值在比较？" |
| c5 | code | switch 示例 | 星期 1-7 + default，展示完整语法 |
| c6 | **animation** | **switch-fallthrough** | BranchPlayer：case 2 匹配后无 break → 穿透到 case 3，输出"BC"。直观展示穿透的后果 |
| c7 | concept | 三目运算符 ? : | 语法糖、对照例子、一句话"简单二选一用 ?:，超过两行用 if/else" |
...
| # | 卡类型 | 标题 | 内容要点 |
|---|--------|------|---------|
| c1 | concept | 指针是什么 | 指针 = 存地址的变量；& 取地址，* 解引用；判断标准："变量里存的是数据还是地址？" |
| c2 | concept | 声明与基本操作 | p vs *p vs &x 三者关系；* 在声明里是类型的一部分，在表达式里是解引用 |
| c3 | **animation** | **指针与解引用** | ScopeCode `pointer-intro`：4 步 — 声明 x → 指针存 &x → *p 读取 → *p=100 修改 |
| c4 | concept | 指针也占内存 | sizeof(任何*) = 8 字节（64位），跟指向什么类型无关 |
| c5 | code | 指针基本操作 | 完整程序：取地址、解引用读取、通过指针修改原变量 |
| c6 | concept | nullptr 空指针 | 解引用空指针 = UB；用前检查 if(p)；C++11 用 nullptr 不用 NULL |
| c7 | concept | const 与指针 | const int* p（指向只读）vs int* const p（指针只读），口诀：const 在 * 左边管目标 |
| c8 | practice | 通过指针改值 | fill: int* p=&x; *p=20; 后 x=? → 20 |
| c9 | practice | const 指针判断 | const int* p; 能改指向不能改值 → p=&y 合法 |
