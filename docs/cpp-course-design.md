...
> 最后更新：2026-05-26 — 基础 17/145 ✅ · 进阶 9/69 ✅ · OOP 2/11 🔨 · 流与文件 1/7 · STL 0/5 · 泛型 0/4 · 现代C++ 0/7 · 工程化 0/5

### 下一步（明天）

3.1 类与对象 ✅、3.2 访问控制与封装 ✅。按设计文档继续推进 3.3~3.11：

| 节点 | 内容 | 预估卡片 |
|------|------|------|
| 3.1 类与对象 | class vs struct、this 指针、const 成员函数 | 7 ✅ |
| 3.2 访问控制与封装 | public/private 分工、BankAccount 实战、getter/setter 的度 | 7 ✅ |
| 3.3 构造与析构 | 初始化列表、explicit、析构函数（虚析构预告）、RAII、构造析构顺序 | 8 |
| 3.4 拷贝控制 | 拷贝构造 vs 拷贝赋值区分、浅拷贝灾难、Rule of Three、深拷贝实现 | 8 |
| 3.5 运算符重载 | `<<` `==` `=` 重载、成员 vs 友元、不改变优先级 | 5 |
| 3.6 分文件编写 | .h/.cpp 分离、include guard、前向声明 | 5 |
| 3.7 编译链接模型 | 预处理→编译→链接、常见链接错误（undefined reference） | 4 |
| 3.8 继承 | is-a、三种继承访问修饰符、构造析构顺序、组合 vs 继承 | 7 |
| 3.9 多态 | virtual、vtable、override/final、对象切片、虚析构函数、纯虚函数 | 9 |
| 3.10 设计原则 | 组合优于继承、面向接口、OOP 不是银弹 | 5 |
| 3.11 实战：成绩管理系统 | 需求分析→Student+Campus→GradeBook→继承→多态→<<重载→改错 | 7 |

当前节奏：每天 1-2 个节点。明天从 3.3 构造与析构开始。
...
| 03  | 面向对象           | 11     | ~69 卡 | 🔨 1/11 — 类与对象 · 访问控制与封装 · 构造析构 · 拷贝控制 · 运算符重载 · 分文件编写 · 编译链接 · 继承 · 多态 · 设计原则 · 实战：成绩管理系统 |
